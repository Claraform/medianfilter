...
void Master()
{
 ...
 for (int j = 1; j <= slaves; j++)
 {
  ...
  //while (!flag)
  //{
  // MPI_Iprobe(j, TAG, MPI_COMM_WORLD, &flag, &stat);
  //}
  int ystart=0;
  int yend=303;
  //MPI_Recv(&ystart, 1, MPI_BYTE, j, TAG, MPI_COMM_WORLD, &stat);
  printf("got \n");
  //MPI_Recv(&yend, 1, MPI_INT, j, TAG, MPI_COMM_WORLD, &stat);
  //MPI_Recv(&numrows, 1, MPI_INT, j, TAG, MPI_COMM_WORLD, &stat);
  printf("Received %d \n", j);
  ...
  {
   MPI_Recv(&received[plswork][0], 15*8, MPI_UNSIGNED_CHAR, j, TAG, MPI_COMM_WORLD, &stat);
  }
...
int median(int* window, int size)
{
 ...
 /*qsort((void*)window, size, sizeof(int), compare);
 for (int i = 0; i<size; i++)
 {
  printf("i in window is %d \n", window[i]);
 }
 */
...
void Slave(int ID){
 ...
 int yend;
 printf("Slave\n");
 // Determine Size of Incoming Message
 // Receiver data
 ...
 */
 ...
 //out[2] = numrows;
 printf("why %d \n", ystart);
 //MPI_Send(&ystart, 1, MPI_BYTE, 0, TAG, MPI_COMM_WORLD);
 printf("why \n");
 ...
 {
  printf("meow \n");
  MPI_Send(&output[GRR][0], 15*8, MPI_UNSIGNED_CHAR, 0, TAG, MPI_COMM_WORLD);
 }
